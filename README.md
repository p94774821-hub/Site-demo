...
sequenceDiagram
    ...
    participant S as Servidor (Railway)
    participant API as /api/content
    participant LS as localStorage

    U->>S: Acessa a página
    S->>U: HTML + CSS + JS
    U->>API: fetch('/api/content')
    API-->>U: JSON com dados do site
    U->>LS: Salva tema e cor primária
    U->>U: Renderiza conteúdo dinâmico
    Note over U,LS: Atualização a cada 30s
```
